...
const assertSameContext = (
  ...
  if (
    ...
    throw new WorkflowOrchestrationError(
      ...
      "Workflow input does not match assessment tenancy.",
      {
...
const approvalForSignal = (
  signalType: WorkflowSignalRequest["signal_type"],
): ApprovalEvent["gate"] | undefined => {
  switch (signalType) {
    case "soa_approved":
      return "soa";
...
export class AssessmentLifecycleOrchestrator {
  ...
  async start(
    ...
    if (existing) {
      ...
      throw new WorkflowOrchestrationError(
        ...
        "Assessment already has an active lifecycle workflow.",
        {
    ...
    await this.audit(run, "lifecycle_workflow_started", input.requested_by, {
      idempotency_key: input.idempotency_key,
    });

    const { run: progressed } = await this.progressFromStart(
      run,
      assessment,
      input,
    );
  ...
  async listByAssessment(assessmentId: string): Promise<WorkflowRunResponse[]> {
    return (await this.deps.workflows.listByAssessment(assessmentId)).map(
      toResponse,
    );
  }
  ...
  async signal(
    ...
    if (
      expectedGate &&
      (!approvalEvent ||
        approvalEvent.gate !== expectedGate ||
        approvalEvent.decision !== "approved")
    ) {
      throw new WorkflowOrchestrationError(
        "APPROVAL_EVENT_REQUIRED",
        "Signal requires a valid approved approval_event.",
        {
    ...
    switch (signal.signal_type) {
      case "framework_selected": {
        updatedRun.state.selected_framework_id = String(
          signal.payload.framework_id ?? "",
        );
        updatedRun.state.scf_version_id = String(
          signal.payload.scf_version_id ??
            updatedRun.state.scf_version_id ??
            "",
        );
        ({ run: updatedRun, assessment: updatedAssessment } =
          await this.advanceToSoaApproval(
            updatedRun,
            updatedAssessment,
            signal.actor_id,
            signal.idempotency_key,
            signal.trace_id ?? updatedRun.state.trace_id,
          ));
        break;
      }
      case "soa_approved":
        ({ run: updatedRun, assessment: updatedAssessment } =
          await this.advanceFromApproval(
            updatedRun,
            updatedAssessment,
            "soa_approved",
            "gap_analysis",
      ...
      case "report_approved":
        ({ run: updatedRun, assessment: updatedAssessment } =
          await this.closeWithReportApproval(
            updatedRun,
            updatedAssessment,
            signal.actor_id,
            signal.idempotency_key,
      ...
      case "assessment_blocked":
        updatedRun = await this.markBlocked(
          updatedRun,
          String(
            signal.payload.blocked_reason ?? "manual_intervention_required",
          ),
          signal.actor_id,
        );
        break;
      case "assessment_cancelled":
        updatedRun = await this.markCancelled(
          updatedRun,
          signal.actor_id,
          "Assessment cancelled by signal.",
        );
        break;
      case "assessment_resumed":
        updatedRun = await this.resumeRun(
          updatedRun,
          signal.actor_id,
          "Workflow resumed by signal.",
        );
        break;
      default:
  ...
  private async progressFromStart(
    ...
    if (currentAssessment.id && currentAssessment.organizationId) {
      try {
        const freshCount = await this.deps.assessments?.getDocumentCount(
          currentAssessment.id,
        );
        if (typeof freshCount === "number") {
          currentAssessment = {
            ...currentAssessment,
            documentCount: freshCount,
          };
        }
      } catch (err) {
    ...
    if (
      currentAssessment.state === "draft" &&
      currentAssessment.documentCount > 0
    ) {
      const progressed = await this.transitionSequence(
        currentRun,
        currentAssessment,
        ["documents_uploaded"],
        input.requested_by,
        `${input.idempotency_key}:documents_uploaded`,
        input.trace_id,
      );
  ...
  private async transitionSequence(
    ...
    for (const nextState of states) {
      ...
      await this.audit(currentRun, "lifecycle_workflow_step_started", actorId, {
      ...
      const result = this.deps.assessmentEngine.transition(
        currentAssessment,
        nextState,
        approvalEvent
          ? {
      ...
      currentAssessment = this.applyPostTransitionFlags(
        result.assessment,
        nextState,
      );
      currentRun.state.assessment_state = currentAssessment.state;
  ...
  private applyPrerequisiteFlags(
    assessment: AssessmentSnapshot,
    nextState: AssessmentState,
  ): void {
    if (nextState === "documents_ingested")
      assessment.requiredDocumentJobsComplete = true;
    if (nextState === "scf_pre_analysis_ready")
      assessment.scfPreAnalysisRegistered = true;
    if (nextState === "framework_selected") assessment.frameworkSelected = true;
    if (nextState === "scope_drafted") assessment.scopeDrafted = true;
    if (nextState === "soa_drafted" || nextState === "soa_under_review")
      assessment.soaDraftVersionComplete = true;
  ...
  private applyPostTransitionFlags(
    assessment: AssessmentSnapshot,
    nextState: AssessmentState,
  ): AssessmentSnapshot {
    ...
    if (nextState === "evidence_analysis_ready")
      updated.evidenceAnalysisReady = true;
    if (nextState === "gap_analysis_approved")
      updated.gapAnalysisApproved = true;
  ...
  private async markBlocked(
    ...
    await this.audit(updated, "lifecycle_workflow_blocked", actorId, {
      blocked_reason: updated.state.blocked_reason,
    });
    return updated;
  }
  ...
  private async markCancelled(
    ...
    updated.updated_at = now();
    updated.state.updated_at = updated.updated_at;
    await this.audit(updated, "lifecycle_workflow_cancelled", actorId, {
      reason,
    });
    return updated;
  }
  ...
  private async resumeRun(
    ...
    if (!["blocked", "failed"].includes(run.status)) {
      throw new WorkflowOrchestrationError(
        ...
        "Only blocked or failed workflows can be resumed.",
        {
          status: run.status,
        },
      );
    ...
    updated.updated_at = now();
    updated.state.updated_at = updated.updated_at;
    await this.audit(updated, "lifecycle_workflow_resumed", actorId, {
      reason,
    });
    return updated;
  }
  ...
  private assertRunAssessmentContext(
    run: WorkflowRunRecord,
    assessment: AssessmentSnapshot,
  ): void {
    if (
      ...
      throw new WorkflowOrchestrationError(
        "ASSESSMENT_CONTEXT_MISMATCH",
        "Workflow run does not match assessment tenancy.",
        {
          workflow_run_id: run.workflow_run_id,
        },
      );
  ...
  private async audit(
    ...
    await this.deps.audit.record({
      ...
      workflow_run_id: run.workflow_run_id as string,
      step_name: run.state.current_step,
      ...(actorId ? { actor_id: actorId } : { system_actor: SYSTEM_ACTOR }),
      trace_id: run.state.trace_id as string,
      timestamp: now(),
      metadata,
    });
  }
