// SettingsSecurity.tsx — API Keys + API Reference tabs content
...
import { AlertTriangle, Check, ChevronDown, ChevronRight, Copy, ExternalLink, Key, Loader2, Trash2 } from "lucide-react"
...
// ─── Method color mapping (design-system tokens) ───────────────
const methodColor: Record<string, { color: string; background: string }> = {
  GET:    { color: "var(--ds-success)",      background: "var(--ds-success-light)" },
  POST:   { color: "var(--ds-accent)",       background: "var(--ds-accent-light)" },
  PATCH:  { color: "var(--ds-warning, #f59e0b)", background: "var(--ds-warning-light, rgba(245,158,11,0.1))" },
  DELETE: { color: "var(--ds-danger, #ef4444)",  background: "var(--ds-danger-light, rgba(239,68,68,0.1))" },
}
...
interface SettingsSecurityProps {
  ...
  setKeyToRevoke: (k: ApiKeySummary | null) => void
...
export function SettingsSecurityKeys({
  activeOrgId,
  apiKeys,
  copiedId,
  onCopy,
  ...
  handleRevokeKey,
}: Omit<SettingsSecurityProps, "activeOrgName" | "expandedGroup" | "setExpandedGroup">) {
  const renderCopyBtn = (text: string, id: string, size: "icon" | "sm" = "icon") => (
    <Button variant="outline" size={size} onClick={() => onCopy(text, id)} className="shrink-0">
      {copiedId === id
        ? <Check className="w-4 h-4" style={{ color: "var(--ds-success)" }} />
      ...
      <CardContent className="space-y-4">
        ...
        {newKey && (
          <div
            ...
            style={{ background: "var(--ds-success-light)", borderColor: "var(--ds-success-light)" }}
          >
            <div className="text-sm font-semibold flex items-center justify-between" style={{ color: "var(--ds-success)" }}>
            ...
            <code className="text-sm font-mono break-all px-2 py-1 rounded block" style={{ background: "var(--ds-success-light)" }}>{newKey}</code>
          </div>
        )}
...
export function SettingsSecurityDocs({
  activeOrgId,
  copiedId,
  onCopy,
  expandedGroup,
  setExpandedGroup,
}: Pick<SettingsSecurityProps, "activeOrgId" | "copiedId" | "onCopy" | "expandedGroup" | "setExpandedGroup">) {
  return (
    <div className="space-y-4">
      ...
      <Card className="border-border bg-card/60">
        ...
        <CardContent>
          <pre
            className="p-4 rounded-lg text-xs font-mono overflow-x-auto border border-border leading-relaxed"
            style={{ background: "var(--ds-code-bg)", color: "var(--ds-code-text)" }}
          >
{`# Every request requires:
curl -X GET "${API_URL}/api/v1/assessments" \\
  -H "Authorization: Bearer <API_KEY>" \\
  -H "x-standard-tenant-id: ${activeOrgId || "<TENANT_ID>"}" \\
  -H "Content-Type: application/json"`}
      ...
      <Card className="border-primary/20 bg-primary/5">
        ...
        <CardContent className="space-y-3">
          <Button
            onClick={() => onCopy(buildLlmSystemPrompt(activeOrgId || "YOUR_TENANT_ID"), "llm-prompt")}
            className="w-full"
          >
            {copiedId === "llm-prompt"
              ? <><Check className="w-4 h-4 mr-2" />Copied to Clipboard!</>
              : <><Copy className="w-4 h-4 mr-2" />Copy Full System Prompt ({Math.round(buildLlmSystemPrompt(activeOrgId || "").length / 1000)}KB)</>
            }
          </Button>
          <details className="text-xs">
            <summary className="cursor-pointer text-muted-foreground hover:text-foreground">Preview system prompt</summary>
            <pre
              className="mt-2 p-3 rounded-lg font-mono overflow-x-auto max-h-64 overflow-y-auto leading-relaxed whitespace-pre-wrap"
              style={{ background: "var(--ds-code-bg)", color: "var(--ds-code-text)" }}
            >
              {buildLlmSystemPrompt(activeOrgId || "YOUR_TENANT_ID")}
            </pre>
          </details>
        </CardContent>
      </Card>
