/**
 * Standard MCP Server — Platform Status Tools
 */
...
export async function handleGetPlatformHealth(
  ...
  try {
    ...
    const inWindow = recentMetrics.filter((m: any) =>
      new Date(m.created_at ?? m.timestamp ?? 0) >= new Date(since)
    );
    const totalRequests = inWindow.reduce((s: number, m: any) => s + (m.total_requests ?? 1), 0);
    const totalErrors = inWindow.reduce((s: number, m: any) => s + (m.error_count ?? 0), 0);
    const avgLatency = inWindow.length
      ? inWindow.reduce((s: number, m: any) => s + (m.avg_latency_ms ?? 0), 0) / inWindow.length
      : 0;
...
export async function handleListSocAlerts(
  ...
  try {
    ...
    const isPlatformAdmin = (ctx.session?.user as Record<string, unknown>)?.["platformAdmin"] === true;
    if (!isPlatformAdmin) {
    ...
    const filtered = alerts.filter((a: any) =>
      new Date(a.created_at ?? 0) >= new Date(since)
    );
