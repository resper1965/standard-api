...
function mapActivityRow(row: typeof privacyProcessingActivities.$inferSelect): PrivacyActivityResponse {
  return {
    ...
    controller_role: row.controllerRole as PrivacyActivityResponse["controller_role"],
    ...
    privacy_regime: ((row.metadata as Record<string, unknown>)?.privacy_regime as PrivacyActivityResponse["privacy_regime"]) ?? "lgpd",
    legal_bases: ((row.metadata as Record<string, unknown>)?.legal_bases as PrivacyActivityResponse["legal_bases"]) ?? [],
    legal_basis_lgpd: row.legalBasisLgpd as PrivacyActivityResponse["legal_basis_lgpd"] ?? null,
...
function mapThirdPartyRow(row: typeof privacyProcessingActivityThirdParties.$inferSelect): PrivacyThirdPartyResponse {
  return {
    ...
    transfer_mechanism: (row.transferMechanism as PrivacyThirdPartyResponse["transfer_mechanism"]) ?? null,
    active: row.active,
    created_at: toIso(row.createdAt),
    updated_at: toIso(row.updatedAt),
  };
...
function mapScfControlRow(row: typeof privacyProcessingActivityScfControls.$inferSelect): PrivacyScfControlResponse {
  return {
    ...
    applicability_status: row.applicabilityStatus as PrivacyScfControlResponse["applicability_status"],
...
const createDrizzleActivityRepository = (db: DbClient): PrivacyActivityRepository => ({
  async save(activity) {
    await db.insert(privacyProcessingActivities).values({
      ...
      metadata: { ...activity.metadata, privacy_regime: activity.privacy_regime, legal_bases: activity.legal_bases },
    }).onConflictDoNothing();
  },
  ...
  async update(activity) {
    await db.update(privacyProcessingActivities)
      .set({
        name: activity.name,
        description: activity.description,
        businessProcess: activity.business_process,
...
const createDrizzleDataSubjectRepository = (db: DbClient): PrivacyDataSubjectRepository => ({
  ...
  async listByActivity(activityId, tenantId) {
  ...
  async remove(id, tenantId) {
    await db.update(privacyProcessingActivityDataSubjects)
      .set({ deletedAt: new Date() })
      .where(and(
...
const createDrizzleDataCategoryRepository = (db: DbClient): PrivacyDataCategoryRepository => ({
  ...
  async remove(id, tenantId) {
    await db.update(privacyProcessingActivityDataCategories)
      .set({ deletedAt: new Date() })
      .where(and(
...
const createDrizzleThirdPartyRepository = (db: DbClient): PrivacyThirdPartyRepository => ({
...
const createDrizzleScreeningRepository = (db: DbClient): PrivacyScreeningRepository => ({
  async save(screening) {
    await db.insert(privacyProcessingActivityScreenings).values({
      ...
      screenedAt: screening.screened_at ? new Date(screening.screened_at) : new Date(),
...
const createDrizzleFieldReviewRepository = (db: DbClient): PrivacyFieldReviewRepository => ({
  async save(review) {
    await db.insert(privacyProcessingActivityFieldReviews).values({
      ...
      reviewedAt: review.reviewed_at ? new Date(review.reviewed_at) : undefined,
    }).onConflictDoNothing();
  },
  async listByActivity(activityId, tenantId) {
  ...
  async update(review) {
    await db.update(privacyProcessingActivityFieldReviews)
      .set({
        reviewStatus: review.review_status,
        reviewerId: review.reviewer_id,
        comment: review.comment,
        reviewedAt: review.reviewed_at ? new Date(review.reviewed_at) : new Date(),
        updatedAt: new Date(),
      })
      .where(and(
...
const createDrizzleScfControlRepository = (db: DbClient): PrivacyScfControlRepository => ({
...
export const createDrizzlePrivacyRepositories = (db: DbClient): PrivacyRepositories => ({
