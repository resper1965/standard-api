...
const toApiError = (error: unknown): never => {
  if (error instanceof AgentRuntimeError) {
    const status = error.code.includes("NOT_FOUND")
      ? 404
      : error.code.includes("NOT_ALLOWED") || error.code.includes("GUARDRAIL")
        ? 409
        : 400;
    throw new ApiError(
...
export const agentRuntimeRoutes: RouteDefinition[] = [
  {
    ...
    handler: () =>
      json({ agents: FUNCTIONAL_AGENT_CONTRACTS, tools: safeTools() }),
  },
  {
    method: "POST",
    ...
    handler: async ({
      ...
      const body =
        validatedBody as import("@standard/schemas").StartAgentRunRequest;
      const locale =
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId }) => {
      ...
      return json(
        await new AgentRuntimeService(deps.agentRuntime).listRuns(
  ...
  },
  {
    ...
    permissions: ["agent:read"],
    handler: async ({ params, deps, organizationId }) => {
      const run = await new AgentRuntimeService(deps.agentRuntime).getRun(
        routeUuidParam(params, "agentRunId"),
        requireOrganizationId({ organizationId }),
      );
  ...
  },
  {
    ...
    handler: async ({ request, params, deps, organizationId, traceId }) => {
      ...
      try {
        return json(
          await new AgentRuntimeService(deps.agentRuntime).invokeTool(
            run.agent_run_id,
            {
  ...
  },
  {
    ...
    handler: async ({ request, params, deps, organizationId, traceId }) => {
      ...
      try {
        const completed = await new AgentRuntimeService(
          deps.agentRuntime,
        ).completeRun(run.agent_run_id, {
          output: body.output,
          context: {
            organization_id: run.organization_id,
            assessment_id: run.assessment_id,
