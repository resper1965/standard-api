    ...
    handler: async ({ request, deps, params, organizationId, actorId, traceId }) => {
      ...
      try {
        ...
        const run = await runtime.startRun({
          agent_id: body.mode === "consultative" ? "standard-consultative-analyst" : "standard-strict-gap-analyst",
          agent_version: "v1",
          prompt_version: "v1",
        ...
        await new AuditEventService(deps.observability).record({
          ...
          outcome: "success",
          trace_id: traceId,
          metadata_safe: { mode: body.mode, text_size: body.raw_text.length, source: "m2m-integration" }
        });
        ...
        return json({
         ...
         if (error instanceof Error) {
            throw new ApiError("INTERNAL_ERROR", `Integration analysis failed: ${error.message}`, 500);
         }
         throw error;
      }
    }
