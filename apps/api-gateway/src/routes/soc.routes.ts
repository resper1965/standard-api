...
export const socRoutes: RouteDefinition[] = [
  ...
  // SOC pipeline health check â€” used by dashboards and on-call runbooks.
  // Platform admin or tenant admin only (security-sensitive aggregate).
  {
    ...
    openapi: {
      ...
      description:
        "Returns SOC monitoring pipeline status: queue binding health, alert service configuration, and pipeline readiness. Requires admin:write permission (platform admin only).",
      responses: {
        200: {
    ...
    handler: async (ctx) => {
      ...
      const note =
        "For detailed security event counts, query the security_events table or Cloudflare Logpush.";
      ...
      try {
        ...
        if (body.asyncCall) {
          ...
          if (ctx.deps.SOC_TRIAGE_QUEUE) {
            ...
            console.warn(
              `[soc:triage] SOC_TRIAGE_QUEUE not bound â€” job ${job_id} will not be processed.`,
            );
          }
          return json(
            { job_id, message: "Accepted for async processing" },
            { status: 202 },
          );
        }
