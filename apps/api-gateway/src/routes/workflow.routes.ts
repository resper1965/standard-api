...
const toApiError = (error: unknown): never => {
  if (error instanceof WorkflowOrchestrationError) {
    ...
    const apiCode: ApiErrorCode =
      error.code === "APPROVAL_EVENT_REQUIRED"
        ? "APPROVAL_EVENT_REQUIRED"
        : error.code === "ASSESSMENT_CONTEXT_MISMATCH"
          ? "INVALID_STATE_TRANSITION"
          : error.code === "DUPLICATE_ACTIVE_WORKFLOW"
            ? "CONFLICT"
            : error.code === "WORKFLOW_NOT_FOUND"
              ? "NOT_FOUND"
              : "VALIDATION_ERROR";
...
const syncAssessmentState = async (
  ...
  assessment.snapshot = {
    ...assessment.snapshot,
    state: assessmentState as AssessmentRecord["snapshot"]["state"],
  };
...
export const workflowRoutes: RouteDefinition[] = [
  {
    ...
    handler: async ({
      ...
      try {
        ...
        await syncAssessmentState(
          deps,
          assessment,
          run.state.assessment_state,
          requireOrganizationId({ organizationId }),
        );
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId }) => {
      ...
      const runs = await new AssessmentLifecycleOrchestrator(
        getTenantWorkflowDeps(deps, requireOrganizationId({ organizationId })),
      ).listByAssessment(assessment.assessment_id);
  ...
  },
  {
    ...
    permissions: ["assessment:read"],
    handler: async ({ params, deps, organizationId }) => {
      const run = await new AssessmentLifecycleOrchestrator(
        getTenantWorkflowDeps(deps, requireOrganizationId({ organizationId })),
      ).get(routeUuidParam(params, "workflowRunId"));
  ...
  },
  {
    ...
    handler: async ({ request, params, deps, organizationId }) => {
      ...
      try {
        return json(
          await new AssessmentLifecycleOrchestrator(
            getTenantWorkflowDeps(
              deps,
              requireOrganizationId({ organizationId }),
            ),
          ).cancel(routeUuidParam(params, "workflowRunId"), body),
        );
      } catch (error) {
        return toApiError(error);
      }
    ...
    handler: async ({ request, params, deps, organizationId }) => {
      ...
      try {
        return json(
          ...
          ).resume(routeUuidParam(params, "workflowRunId"), body),
        );
      } catch (error) {
        return toApiError(error);
      }
  ...
  },
  {
    ...
    handler: async ({ request, params, deps, organizationId, traceId }) => {
      ...
      const run = await orchestrator.get(
        routeUuidParam(params, "workflowRunId"),
      );
      if (!run) throw new ApiError("NOT_FOUND", "Workflow run not found.", 404);
      const assessment = await requireAssessment(
        deps,
        run.state.assessment_id,
        requireOrganizationId({ organizationId }),
      );
      ...
      try {
        ...
        const updated = await orchestrator.get(
          routeUuidParam(params, "workflowRunId"),
        );
        if (updated)
          await syncAssessmentState(
            deps,
            assessment,
            updated.state.assessment_state,
            requireOrganizationId({ organizationId }),
          );
        await new AuditEventService(deps.observability).record({
          organization_id: assessment.organization_id,
