...
export const reportingRoutes: RouteDefinition[] = [
  {
    ...
    handler: async ({
      ...
      try {
        const draft = await new ReportDraftService(
          deps.reporting,
        ).createReportDraft(
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      return json(
        await new ReportDraftService(deps.reporting).listReportVersions(
          assessment.assessment_id,
  ...
  },
  {
    ...
    permissions: ["report:read"],
    handler: async ({ params, deps, organizationId, traceId }) => {
      const report = await deps.reporting.repositories.versions.get(
        routeUuidParam(params, "reportVersionId"),
        requireOrganizationId({ organizationId }),
      );
      if (!report)
        throw new ApiError(
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      return json(
        await new ReportComposerService(
          deps.reporting,
        ).composeFullAssessmentReport(
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      return json(
        await new ReportStorageService(deps.reporting).listArtifacts(
          report.report_version_id,
          contextFor(assessment, traceId),
        ),
      );
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      return json(
        await new ReportValidationService(
          deps.reporting,
        ).validateReportForReview(
          report.report_version_id,
          contextFor(assessment, traceId),
        ),
      );
    ...
    handler: async ({
      ...
      const rendered =
        body.format === "markdown"
          ? await service.renderMarkdown(
              report.report_version_id,
              contextFor(assessment, traceId, actorId!),
            )
          : await service.renderJson(
              report.report_version_id,
              contextFor(assessment, traceId, actorId!),
            );
      const artifact = body.store_artifact
        ? await new ReportStorageService(deps.reporting).storeArtifact(
            report.report_version_id,
            rendered,
            contextFor(assessment, traceId, actorId!),
          )
  ...
  },
  {
    ...
    handler: async ({
      ...
      try {
        return json(
          await new ReportReviewService(deps.reporting).submitReportForReview(
            report.report_version_id,
            contextFor(assessment, traceId, actorId!),
          ),
        );
  ...
  },
  {
    ...
    handler: async ({
      ...
      const approval = await deps.approvals
        .withOrganization(requireOrganizationId({ organizationId }))
        .getForGate(body.approval_event_id, "report");
      if (!approval)
        throw new ApiError(
          "APPROVAL_REQUIRED",
          "A valid report approval_event is required.",
          409,
        );
      try {
  ...
  },
  {
    ...
    handler: async ({
      ...
      return json(
        await new ReportDraftService(deps.reporting).regenerateReportDraft(
          report.report_version_id,
          {},
          contextFor(assessment, traceId, actorId!),
        ),
  ...
  },
  {
    ...
    permissions: ["report:read"],
    handler: async ({ params, deps, organizationId, traceId }) => {
      const artifact = await deps.reporting.repositories.artifacts.get(
        routeUuidParam(params, "artifactId"),
        requireOrganizationId({ organizationId }),
      );
      if (!artifact)
        throw new ApiError(
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId, actorId }) => {
      ...
      return json({
        download_url: await new ReportStorageService(
          deps.reporting,
        ).generateDownloadUrl(
  ...
  },
  {
    ...
    handler: async ({
      ...
      return json(
        await new ExportJobService(deps.reporting).requestExport(
          assessment.assessment_id,
          body.format,
          body.report_type,
          contextFor(assessment, traceId, actorId!),
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      return json(
        await new ExportJobService(deps.reporting).listExportJobs(
          assessment.assessment_id,
          contextFor(assessment, traceId),
        ),
      );
  ...
  },
  {
    ...
    permissions: ["report:read"],
    handler: async ({ params, deps, organizationId, traceId }) => {
      const job = await deps.reporting.repositories.exportJobs.get(
        routeUuidParam(params, "exportJobId"),
        requireOrganizationId({ organizationId }),
      );
      if (!job)
        throw new ApiError(
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, actorId, traceId }) => {
      ...
      let rendered;
      if (format === "markdown") {
        rendered = await service.renderMarkdown(
          report.report_version_id,
          contextFor(assessment, traceId, actorId!),
        );
      } else if (format === "pdf") {
        rendered = await service.renderPdf(
          report.report_version_id,
          contextFor(assessment, traceId, actorId!),
        );
      } else if (format === "docx") {
        rendered = await service.renderDocx(
          report.report_version_id,
          contextFor(assessment, traceId, actorId!),
        );
      } else {
        rendered = await service.renderJson(
          report.report_version_id,
          contextFor(assessment, traceId, actorId!),
        );
      }
  ...
  },
  {
    ...
    handler: async ({
      ...
      const locale = new URL(request.url).searchParams.get("locale") as
        | import("@standard/schemas").SupportedLocale
        | null;
      try {
        const result = await new AuditPackageService(
          deps.reporting,
        ).generatePackage(
          assessment.assessment_id,
          contextFor(assessment, traceId, actorId!),
  ...
  },
  {
    ...
    handler: async ({ params, deps, organizationId, traceId }) => {
      ...
      if (job.status !== "succeeded")
        throw new ApiError(
          "EXPORT_JOB_FAILED",
          `Export job status: ${job.status}. Only succeeded jobs can be downloaded.`,
          409,
        );
      const assessment = await requireAssessment(
        deps,
      ...
      return json({
        ...
        download_url: job.report_version_id
          ? await new ReportStorageService(deps.reporting).generateDownloadUrl(
