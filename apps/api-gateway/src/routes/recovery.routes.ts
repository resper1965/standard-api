  ...
  },
  {
    ...
    handler: async (context) => {
      ...
      try {
        ...
        // Only allow bootstrap when no platform_admin exists
        ...
        const existingUsers = await authDb.execute(
          sql`SELECT id, email_verified FROM public."user" WHERE email = ${body.email} LIMIT 1`,
        );
        ...
        if (existingUsers.rows.length > 0) {
          ...
          userId = existing.id;
          // Update password and set verified + platform_admin
          await authDb.execute(
