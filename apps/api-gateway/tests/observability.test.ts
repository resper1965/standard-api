...
test("audit logs endpoint retorna eventos do assessment para customer autenticado", async () => {
  ...
  expect(
    allowed.body.data.some(
      (event: any) => event.action === "agent_run_started",
    ),
  ).toBe(true);

  // Unauthenticated request should still be denied
...
test("permission denied em admin route gera security event consultável por admin", async () => {
  const client = createTestClient();
  const created = await client.createAssessment(1);

  // Customer trying admin-only route should be denied
  const denied = await client.send(
  ...
  expect(
    events.body.data.some(
      (event: any) => event.event_type === "forbidden_access_attempt",
    ),
  ).toBe(true);
});
...
test("KB search registra métricas e usage sem query integral", async () => {
  ...
  const metrics = await client.send(
    `/api/v1/assessments/${created.assessmentId}/metrics`,
    "GET",
    undefined,
    {
      "x-standard-tenant-id": created.organizationId,
  ...
  expect(
    metrics.body.data.some(
      (metric: any) => metric.metric_name === "kb_search_count",
    ),
  ).toBe(true);
  expect(JSON.stringify(metrics.body.data).includes("access control")).toBe(
    false,
  );

  const usage = await client.send(
    `/api/v1/assessments/${created.assessmentId}/usage`,
    "GET",
    undefined,
    {
      "x-standard-tenant-id": created.organizationId,
  ...
  expect(
    usage.body.usage.some((record: any) => record.service_name === "vectorize"),
  ).toBe(true);
});
...
test("agent complete registra usage quando tokens são informados", async () => {
  ...
  const run = await client.send(
    ...
    "POST",
    {
      ...
      scf_version_id: ids.scfVersionId,
      input: {
        source_gap_analysis_version_id: "77777777-7777-4777-8777-777777777777",
      },
    },
    headers,
  );
  ...
  await client.send(
    `/api/v1/agent-runs/${run.body.agent_run_id}/complete`,
    "POST",
    {
      output: {
  ...
  expect(
    usage.body.agent_usage.some((record: any) => record.total_tokens === 15),
  ).toBe(true);
});
