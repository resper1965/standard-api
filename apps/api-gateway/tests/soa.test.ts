...
const createSoaDraft = async () => {
  ...
  const draft = await setup.client.send(
    `/api/v1/assessments/${setup.created.assessmentId}/soa/draft`,
    "POST",
    {
    ...
    {
      "x-standard-tenant-id": setup.created.organizationId,
      "x-standard-actor-id": ids.actorId,
    },
  );
...
test("POST scope cria escopo draft multi-tenant", async () => {
  ...
  const scopes = await client.send(
    `/api/v1/assessments/${created.assessmentId}/scope`,
    "GET",
    undefined,
    {
  ...
  expect(scopes.body.data[0].organization_id).toBe(created.organizationId);
});
...
test("PATCH SoA item bloqueia not_applicable sem justificativa", async () => {
  ...
  const patched = await client.send(
    `/api/v1/soa/items/${items.body.data[0].soa_item_id}`,
    "PATCH",
    {
