...
export const createTestClient = () => {
  ...
  const send = async (
    ...
    const idempotencyHeaders =
      mutating && !headers["idempotency-key"]
        ? { "idempotency-key": crypto.randomUUID() }
        : {};
    // Auto-inject CSRF token for mutating methods (M3 CSRF protection)
    const csrfToken =
      "test-csrf-token-" + crypto.randomUUID().replace(/-/g, "").slice(0, 16);
...
export const getTestDb = async () => {
  ...
  let migrationsFolder = path.resolve(
    currentDir,
    "infra/docker/postgres/migrations",
  );
  while (
  ...
  for (const file of files) {
    ...
    if (file === "0016_poam_workflow.sql") {
      sql = sql.replace(
        ...
        "UPDATE poam_items SET poam_code = 'DEFAULT' WHERE poam_code IS NULL;",
      );
      sql = sql.replace(
        ...
        "UPDATE poam_items SET related_gap_finding_id = NULL WHERE related_gap_finding_id IS NULL;",
      );
    }
