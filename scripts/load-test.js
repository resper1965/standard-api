...
export default function () {
  ...
  if (API_KEY && ORG_ID) {
    const authHeaders = {
      ...
      'x-standard-organization-id': ORG_ID,
    };

    // Read-heavy: Get SCF Controls List
    const controlsRes = http.get(`${BASE_URL}/api/v1/scf/controls?limit=25`, { headers: authHeaders });
    check(controlsRes, {
    ...
    const createRes = http.post(`${BASE_URL}/api/v1/assessments`, createPayload, { headers: authHeaders });
    check(createRes, {
      'create assessment status is 201': (r) => r.status === 201,
      'create assessment response time < 400ms': (r) => r.timings.duration < 400,
    });
  }
