...
if (!clientCode.includes('class TenantsResource')) {
  clientCode = clientCode.replace(/(\/\/ ── Helper ──────────────────────────────────────────)/, missingClasses + '\n$1');
}

// 4. Inject standard fields in StandardClient
if (!clientCode.includes('readonly tenants: TenantsResource;')) {
...
if (!clientCode.includes('this.tenants = new TenantsResource(this);')) {
...
if (!clientCode.includes('validate(versionId: string, opts?: RequestOptions)')) {
  clientCode = injectMethods(clientCode, 'GapAnalysisResource', missingMethods.GapAnalysisResource);
}
if (!clientCode.includes('refreshEvidence(versionId: string, opts?: RequestOptions)')) {
  clientCode = injectMethods(clientCode, 'SoaResource', missingMethods.SoaResource);
}
if (!clientCode.includes('deleteOrg(orgId: string, opts?: RequestOptions)')) {
  clientCode = injectMethods(clientCode, 'OrganizationsResource', missingMethods.OrganizationsResource);
}
if (!clientCode.includes('automationRules(id: string, opts?: RequestOptions)')) {
  clientCode = injectMethods(clientCode, 'AssessmentsResource', missingMethods.AssessmentsResource);
}
if (!clientCode.includes('submitForEmbedding(docId: string, opts?: RequestOptions)')) {
  clientCode = injectMethods(clientCode, 'DocumentsResource', missingMethods.DocumentsResource);
}

fs.writeFileSync('packages/sdk/src/client.ts', clientCode, 'utf8');

console.log('client.ts patched forcefully and correctly.');
