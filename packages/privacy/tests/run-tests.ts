...
const run = async () => {
...

  {
    ...
    assert(
      "third_party_sharing=true without parties blocks submission",
      before.can_be_submitted_for_review === false,
    );

    // Add third parties
    const parties = await crud.addThirdParties(
      activity.id,
      [
        {
          name: "Mailchimp",
        ...
        },
        {
          ...
          role: "processor",
          country: "US",
          data_shared: ["page_views", "user_agent"],
        },
      ],
      ctx,
    );
    ...
    assert(
      "party has transfer mechanism",
      parties[0].transfer_mechanism === "standard_contractual_clauses",
    );
    ...
    assert(
      "third_party_sharing=true WITH parties passes coherence",
      after.can_be_submitted_for_review === true,
    );
...

  {
    ...
    assert(
      "TIA is required (international transfer)",
      tia?.result === "required",
    );
    ...
    assert(
      "low-risk: no screenings required",
      lowResults.every((r) => r.result === "not_required"),
    );
  }
...

  {
    ...
    const review = await crud.addFieldReview(
      activity.id,
      {
        field_name: "purpose",
    ...
    assert(
      "field review starts as pending",
      review.review_status === "pending",
    );
    assert(
      "field review source is ai_suggestion",
      review.source === "ai_suggestion",
    );

    // Approve it
    const approved = await crud.updateFieldReview(
      review.id,
      {
    ...
    assert(
      "field review can be approved",
      approved.review_status === "approved",
    );
    assert("reviewer tracked", approved.reviewer_id === ACTOR);

    // Reject another
    const review2 = await crud.addFieldReview(
      activity.id,
      {
    ...
    const rejected = await crud.updateFieldReview(
      review2.id,
      {
...

  {
    ...
    assert(
      "AI does NOT assert compliance",
      result.compliance_assertion === false,
    );
    ...
    const categories = await crud.listDataCategories(
      result.activity.id,
      TENANT,
    );
    assert(
      "AI extracts data categories (email, nome, CPF)",
      categories.length >= 2,
    );
    ...
    assert(
      "field reviews are ai_suggestion source",
      reviews.some((r) => r.source === "ai_suggestion"),
    );
    assert(
      "critical fields get system_rule review",
      reviews.some((r) => r.source === "system_rule"),
    );

    // Pending questions
    assert(
      "AI identifies remaining questions",
      Array.isArray(result.pending_questions),
    );

    // CPF should trigger sensitive data warning
    const sensitiveCategories = categories.filter(
      (c) => c.sensitivity === "sensitive",
    );
...

  {
    ...
    assert(
      "English extracts retention",
      result.activity.retention_period !== null,
    );
  }
...

  {
    ...
    const jsonReport = await report.generateReport(
      extracted.activity.id,
      TENANT,
      "json",
    );
    assert("JSON report generated", jsonReport.report_id !== undefined);
    assert(
      "report NEVER asserts compliance",
      jsonReport.compliance_assertion === false,
    );
    assert("report has completeness score", jsonReport.completeness.score > 0);
    assert(
      "report has executive summary",
      jsonReport.executive_summary.length > 0,
    );
    assert(
      "report tracks field origins",
      Object.values(jsonReport.activity).some(
        (f) => f.origin === "ai_suggested",
      ),
    );
    assert("report includes screenings", jsonReport.screenings.length > 0);
    assert(
      "report includes gaps or warnings",
      jsonReport.gaps.length > 0 || jsonReport.warnings.length > 0,
    );

    // Generate Markdown report
    const mdReport = await report.generateReport(
      extracted.activity.id,
      TENANT,
      "markdown",
    );
    assert(
      ...
      mdReport.markdown !== undefined && mdReport.markdown.length > 0,
    );
    assert(
      "markdown contains disclaimer",
      mdReport.markdown!.includes("does NOT assert compliance") ||
        mdReport.markdown!.includes("does NOT constitute"),
    );
    assert(
      "markdown has evidence matrix",
      mdReport.markdown!.includes("Evidence Matrix"),
    );
  }
...

  {
    ...
    const actA = await crud.createActivity(
      { name: "Tenant A Activity" },
      { organizationId: TENANT_A, actorId: ACTOR, traceId: "t" },
    );
    const actB = await crud.createActivity(
      { name: "Tenant B Activity" },
      { organizationId: TENANT_B, actorId: ACTOR, traceId: "t" },
    );

    // Cross-tenant access denied
    assert(
      "tenant A cannot see tenant B activity",
      (await crud.getActivity(actB.id, TENANT_A)) === null,
    );
    assert(
      "tenant B cannot see tenant A activity",
      (await crud.getActivity(actA.id, TENANT_B)) === null,
    );

    // Tenant A can see own
    assert(
      "tenant A sees own activity",
      (await crud.getActivity(actA.id, TENANT_A))?.id === actA.id,
    );

    // Third parties isolated
    await crud.addThirdParties(actA.id, [{ name: "Vendor A" }], {
    ...
    await crud.addFieldReview(
      actA.id,
      { field_name: "test", source: "human" },
      { organizationId: TENANT_A, actorId: ACTOR, traceId: "t" },
    );
...

  {
    ...
    const { createInMemoryScfRepository, createSyntheticScfFixture } =
      await import("@standard/scf-core");
    const { PrivacyScfBridge, PrivacyScreeningService: ScreenSvc } =
      await import("../src");
    ...
    scfFixture.controls.push(
      ...
      },
      {
        ...
        control_description:
          "Assess transfer impact for international transfers.",
        status: "active",
        is_synthetic: true,
      },
    );
    ...
    scfFixture.requirements.push(
      {
        id: lgpdReq1Id,
        scf_framework_id: lgpdFrameworkId,
        requirement_code: "LGPD-Art38",
        requirement_title: "RIPD",
        requirement_text: "Relatório de Impacto",
        sort_order: 1,
        status: "active",
        is_synthetic: true,
      },
      {
        id: lgpdReq2Id,
        scf_framework_id: lgpdFrameworkId,
        requirement_code: "LGPD-Art33",
        requirement_title: "Transferência Internacional",
        requirement_text: "Transferência de dados",
        sort_order: 2,
        status: "active",
        is_synthetic: true,
      },
    ...
    scfFixture.mappings.push(
      {
        id: "30000000-0000-4000-8000-000000000930",
        scf_version_id: SCF_VER_ID,
        scf_framework_id: lgpdFrameworkId,
        scf_framework_requirement_id: lgpdReq1Id,
        scf_control_id: priControl06Id,
      ...
      {
        id: "30000000-0000-4000-8000-000000000931",
        scf_version_id: SCF_VER_ID,
        scf_framework_id: lgpdFrameworkId,
        scf_framework_requirement_id: lgpdReq2Id,
        scf_control_id: priControl09Id,
        relationship_type: "intersects",
        relationship_strength: "source-defined",
        mapping_source: "synthetic",
        is_official: true,
    ...
    assert(
      "framework code is BR-LGPD",
      lgpdFramework?.framework_code === "BR-LGPD",
    );
    assert(
      "framework jurisdiction is Americas",
      lgpdFramework?.jurisdiction === "Americas",
    );

    const gdprFramework = await bridge.resolveFramework("gdpr");
    assert(
      "bridge returns null for unregistered GDPR framework",
      gdprFramework === null,
    );
    ...
    assert(
      "PRI-06 exists",
      priControls.some((c) => c.control_code === "PRI-06"),
    );
    assert(
      "PRI-01 exists",
      priControls.some((c) => c.control_code === "PRI-01"),
    );
    assert(
      "PRI-09 exists",
      priControls.some((c) => c.control_code === "PRI-09"),
    );

    // ─── Bridge: getPrivacyControlsForRegime ──────────────────────
    const lgpdPriControls = await bridge.getPrivacyControlsForRegime(
      "lgpd",
      SCF_VER_ID,
    );
    assert("LGPD has 2 mapped PRI controls", lgpdPriControls.length === 2);
    assert(
      "PRI-06 is mapped to LGPD",
      lgpdPriControls.some((c) => c.control.control_code === "PRI-06"),
    );
    assert(
      "PRI-09 is mapped to LGPD",
      lgpdPriControls.some((c) => c.control.control_code === "PRI-09"),
    );
    ...
    assert(
      "anchor has scf_version_label",
      anchor?.scf_version_label !== undefined,
    );
    assert(
      "anchor has framework_code=BR-LGPD",
      anchor?.framework_code === "BR-LGPD",
    );
    assert(
      "anchor has 2 mapped controls",
      anchor?.anchored_controls.length === 2,
    );

    // ─── Bridge: isControlMappedToRegime ──────────────────────────
    const pri06mapped = await bridge.isControlMappedToRegime(
      "PRI-06",
      "lgpd",
      SCF_VER_ID,
    );
    ...
    const pri01mapped = await bridge.isControlMappedToRegime(
      "PRI-01",
      "lgpd",
      SCF_VER_ID,
    );
    assert(
      "PRI-01 is NOT mapped to LGPD (no mapping added)",
      pri01mapped === false,
    );
    ...
    assert(
      "DPIA result has scf_anchor",
      (dpiaResult as any).scf_anchor !== undefined,
    );
    assert(
      "DPIA scf_anchor has framework_code=BR-LGPD",
      (dpiaResult as any).scf_anchor?.framework_code === "BR-LGPD",
    );
    assert(
      "DPIA scf_anchor references PRI-06",
      (dpiaResult as any).scf_anchor?.controls?.some(
        (c: any) => c.control_code === "PRI-06",
      ),
    );

    const tiaResult = results.find((r) => r.screening_type === "tia")!;
    assert(
      "TIA result has scf_anchor",
      (tiaResult as any).scf_anchor !== undefined,
    );
    assert(
      "TIA scf_anchor references PRI-09",
      (tiaResult as any).scf_anchor?.controls?.some(
        (c: any) => c.control_code === "PRI-09",
      ),
    );
    ...
    assert(
      "without SCF: DPIA still triggered",
      noScfResults.some(
        (r) => r.screening_type === "dpia" && r.result === "required",
      ),
    );
    assert(
      "without SCF: no scf_anchor attached",
      (noScfResults[0] as any).scf_anchor === undefined,
    );
  }
