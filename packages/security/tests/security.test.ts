...
test("PolicyEngine nega permissão ausente e permite organization_admin", () => {
  const engine = new PolicyEngine();

  // Actor com permissões explícitas vazias não pode aprovar
  ...
  // organization_admin tem soa:approve
  const allowed = engine.authorize({
    auth: {
      ...
      organization_ids: [ids.orgId],
      roles: ["organization_admin"],
      permissions: DEFAULT_ROLE_PERMISSIONS.organization_admin,
      auth_method: "mock_dev",
      issued_at: "2026-04-28T20:00:00.000Z",
      trace_id: "trace-test-0001",
    },
