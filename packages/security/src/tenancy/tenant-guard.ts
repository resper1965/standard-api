...
export class TenantGuard {
  validateBodyTenant(body: unknown, tenant: SecurityTenantContext): AccessDecision {
    ...
    if (typeof value.organization_id === "string" && value.organization_id !== tenant.organization_id) {
  ...
  validateAssessmentAccess(
    ...
    if (assessment.organization_id !== tenant.organization_id) return decision(false, "tenant_mismatch", tenant.trace_id);
