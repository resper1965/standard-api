...
import { renderDocxArtifact } from "../renderers/docx-renderer.placeholder";
...
export class ReportRendererService {
  ...
  async renderJson(reportVersionId: string, context: ReportingContext): Promise<RenderReportResponse> {
    return this.render(reportVersionId, "json", context);
  }

  async renderMarkdown(reportVersionId: string, context: ReportingContext): Promise<RenderReportResponse> {
    return this.render(reportVersionId, "markdown", context);
  }

  async renderHtml(reportVersionId: string, context: ReportingContext): Promise<RenderReportResponse> {
    const markdown = await this.renderMarkdown(reportVersionId, context);
    return { ...markdown, format: "html", mime_type: "text/html", content: `<pre>${markdown.content.replaceAll("<", "&lt;")}</pre>` };
  }
