...
const parseControlRow = (row: Record<string, string>, ctx: ImportContext) => {
  ...
  const domainId = row.scf_domain_id || ctx.domainByCode.get(row.domain_code ?? "");
  if (!domainId) {
    ctx.warnings.push(`Control ${controlCode} skipped because domain was not found.`);
    return;
  }
  const id = row.id || newId();
...
const parseRequirementRow = (row: Record<string, string>, ctx: ImportContext) => {
  const requirementCode = row.requirement_code ?? "";
  const frameworkId = row.scf_framework_id || ctx.frameworkByCode.get(row.framework_code ?? "");
  if (!frameworkId) {
    ctx.warnings.push(`Requirement ${requirementCode} skipped because framework was not found.`);
    return;
  }
  const id = row.id || newId();
...
const parseMappingRow = (row: Record<string, string>, ctx: ImportContext) => {
  const frameworkId = row.scf_framework_id || ctx.frameworkByCode.get(row.framework_code ?? "");
  const requirementId = row.scf_framework_requirement_id || ctx.requirementByCode.get(row.requirement_code ?? "");
  const controlId = row.scf_control_id || ctx.controlByCode.get(row.control_code ?? "");
  if (!frameworkId || !requirementId || !controlId) {
    ctx.warnings.push(`Mapping ${row.requirement_code || row.scf_framework_requirement_id} skipped because relationship targets were not found.`);
    return;
  }
  const rel = row.relationship_type;
  const relationship_type = rel === "equal" || rel === "subset" || rel === "superset" ? rel : "related";
...
export const createCsvScfImporter = (): ScfImporter => ({
  ...
  parse: async (source: ScfImportSource) => {
    ...
    const ctx: ImportContext = {
      versions: [], domains: [], controls: [], frameworks: [], requirements: [], mappings: [], warnings: [],
      versionId: newId(), sourceHash, source,
      domainByCode: new Map(), controlByCode: new Map(), frameworkByCode: new Map(), requirementByCode: new Map()
    };
    ...
    const importRun: ScfImportRun = {
      ...
      import_statistics: {
        ...
        synthetic_records: [...ctx.versions, ...ctx.domains, ...ctx.controls, ...ctx.frameworks, ...ctx.requirements, ...ctx.mappings].filter((item) => item.is_synthetic).length
      },
    ...
    return {
      dataset: { 
        versions: ctx.versions, domains: ctx.domains, controls: ctx.controls, 
        frameworks: ctx.frameworks, requirements: ctx.requirements, mappings: ctx.mappings, 
        strmRelationships: [], importRuns: [importRun] 
      },
