...
type PrerequisiteRule = {
  check: (a: AssessmentSnapshot) => boolean;
  label: string;
};
...
const prerequisiteRules: Partial<Record<AssessmentState, PrerequisiteRule[]>> =
  {
    ...
    documents_ingested: [
      {
        check: (a) => a.requiredDocumentJobsComplete,
        label: "required_document_jobs_complete_or_skipped",
      },
    ],
    scf_pre_analysis_ready: [
      {
        check: (a) => a.scfPreAnalysisRegistered,
        label: "scf_pre_analysis_registered",
      },
    ],
    framework_selected: [
      { check: (a) => a.frameworkSelected, label: "framework_selected" },
    ],
    scope_drafted: [{ check: (a) => a.scopeDrafted, label: "scope_drafted" }],
    soa_drafted: [
      {
        check: (a) => a.soaDraftVersionComplete,
        label: "complete_draft_soa_version",
      },
    ],
    soa_under_review: [
      {
    ...
    soa_ingested: [{ check: (a) => a.soaApproved, label: "soa_approved" }],
    ...
    gap_analysis_drafted: [
      ...
      {
        check: (a) => a.gapAnalysisDrafted,
        label: "gap_analysis_draft_version",
      },
    ],
    ...
    maturity_assessed: [
      ...
      {
        check: (a) => a.maturityAssessed,
        label: "maturity_assessment_version",
      },
    ],
...
export const missingPrerequisites = (
  ...
  return rules.filter((r) => !r.check(assessment)).map((r) => r.label);
};
