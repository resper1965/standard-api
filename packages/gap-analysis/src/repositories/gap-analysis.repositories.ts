...
export class InMemoryEvidenceFindingRepository implements EvidenceFindingRepository {
  ...
  async listByAssessment(
    ...
    return [...this.records.values()].filter(
      (finding) =>
        finding.assessment_id === assessmentId &&
        finding.organization_id === organizationId,
    );
  }
  ...
  withOrganization(organizationId: string) {
    return {
      ...
      get: async (evidenceFindingId: string) =>
      ...
      findBySoaItem: async (soaItemId: string) =>
...
export class InMemoryEvidenceSourceRepository implements EvidenceSourceRepository {
  ...
  async saveMany(sources: EvidenceSourceResponse[]): Promise<void> {
    for (const source of sources)
      this.records.set(source.evidence_source_id, source);
  }
  ...
  async listByFinding(
    ...
    return [...this.records.values()].filter(
      (source) =>
        source.evidence_finding_id === evidenceFindingId &&
  ...
  withOrganization(organizationId: string) {
    return {
      saveMany: async (sources: EvidenceSourceResponse[]) =>
        this.saveMany(sources),
      listByFinding: async (evidenceFindingId: string) =>
...
export class InMemoryGapAnalysisVersionRepository implements GapAnalysisVersionRepository {
  ...
  async listByAssessment(
    ...
    return [...this.records.values()].filter(
      (version) =>
        version.assessment_id === assessmentId &&
  ...
  withOrganization(organizationId: string) {
    return {
      ...
      update: async (version: GapAnalysisVersionResponse) =>
        this.update(version),
      get: async (gapAnalysisVersionId: string) =>
...
export class InMemoryGapFindingRepository implements GapFindingRepository {
  ...
  async listByVersion(
    ...
    return [...this.records.values()].filter(
      (finding) =>
        finding.gap_analysis_version_id === gapAnalysisVersionId &&
        finding.organization_id === organizationId,
    );
  }

  async delete(gapFindingId: string, organizationId: string): Promise<void> {
    const finding = this.records.get(gapFindingId);
    if (finding && finding.organization_id === organizationId) {
      this.records.delete(gapFindingId);
    }
  }

  withOrganization(organizationId: string) {
    return {
      saveMany: async (findings: GapFindingResponse[]) =>
        this.saveMany(findings),
      update: async (finding: GapFindingResponse) => this.update(finding),
      get: async (gapFindingId: string) =>
        this.get(gapFindingId, organizationId),
      listByVersion: async (gapAnalysisVersionId: string) =>
        this.listByVersion(gapAnalysisVersionId, organizationId),
      delete: async (gapFindingId: string) =>
        this.delete(gapFindingId, organizationId),
    };
  }
}
