...
export class GapApprovalService {
  ...
  async validateGapAnalysisForApproval(
    gapAnalysisVersionId: string,
    context: GapAnalysisContext,
  ): Promise<GapAnalysisValidationResponse> {
    return new GapValidationService(this.deps).validateGapAnalysisForReview(
      gapAnalysisVersionId,
      context,
    );
  }

  async approveGapAnalysis(
    gapAnalysisVersionId: string,
    ...
    await this.supersedePreviousApprovedGapAnalysis(
      approved.assessment_id,
      approved.gap_analysis_version_id,
      context,
    );
    await this.logFindingsToLedger(approved, context);
    return approved;
  }

  async supersedePreviousApprovedGapAnalysis(
    assessmentId: string,
    approvedGapAnalysisVersionId: string,
    context: GapAnalysisContext,
  ): Promise<void> {
    const versions = await this.deps.repositories.gapVersions.listByAssessment(
      assessmentId,
      context.organizationId,
    );
    for (const version of versions) {
      if (
        version.gap_analysis_version_id !== approvedGapAnalysisVersionId &&
  ...
  private async logFindingsToLedger(
    ...
    for (const finding of findings) {
      if (!finding.scf_control_id) continue;
      await this.deps.ledger.appendEvent({
        ...
        newValue: finding as unknown as Record<string, unknown>,
