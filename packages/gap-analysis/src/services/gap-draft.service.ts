...
export class GapDraftService {
  ...
  async createGapAnalysisDraft(
    ...
    const items = await this.deps.soa.repositories.items.listByVersion(
      soaVersionId,
      context.organizationId,
    );
    ...
    for (const item of items) {
      ...
      findings.push(
        await this.generateGapFindingForSoaItem(
          item,
  ...
  async generateGapFindingForSoaItem(
    ...
    return {
      ...
      gap_analysis_version_id:
        version?.gap_analysis_version_id ?? crypto.randomUUID(),
      ...
      ...(soaItem.scf_control_id
        ...
        : {}),
      ...(evidenceFinding?.evidence_finding_id
  ...
  async regenerateGapAnalysisDraft(
    gapAnalysisVersionId: string,
    _options: Record<string, unknown>,
    context: GapAnalysisContext,
  ): Promise<GapAnalysisVersionResponse> {
    const version = await this.getGapVersion(gapAnalysisVersionId, context);
    return this.createGapAnalysisDraft(
      version.assessment_id,
      version.source_soa_version_id,
      context,
    );
  }

  async listGapFindings(
    gapAnalysisVersionId: string,
    filters: GapFindingFilters,
    context: GapAnalysisContext,
  ): Promise<GapFindingResponse[]> {
  ...
  private assess(
    soaItem: SoaItemResponse,
    evidenceFinding?: Partial<EvidenceFindingResponse>,
  ): {
    ...
    if (soaItem.applicability_status === "not_applicable") {
      ...
      return {
        ...
        summary: justified
          ? "Control is marked not applicable with rationale."
          : "Control is marked not applicable without sufficient rationale.",
        ...(soaItem.non_applicability_rationale
          ? { rationale: soaItem.non_applicability_rationale }
          : {}),
    ...
    if (
      !evidenceFinding ||
      evidenceFinding.evidence_strength === "absent" ||
      evidenceFinding.evidence_status === "not_evidenced"
    ) {
      return {
        ...
        recommendation:
          "Request or upload evidence before concluding implementation status.",
        requiresUserValidation: true,
      };
    }
    if (
      evidenceFinding.evidence_status === "conflicting" ||
      evidenceFinding.evidence_strength === "conflicting"
    ) {
      return {
    ...
    if (evidenceFinding.evidence_strength === "partial") {
      return {
        ...
        recommendation:
          "Review evidence coverage and request missing artifacts if needed.",
        requiresUserValidation: true,
      };
    }
    if (
      evidenceFinding.evidence_strength === "strong" &&
      (evidenceFinding.confidence_score ?? 0) >= 0.8
    ) {
      return {
        status: "met",
        gapType: "no_gap",
        severity: "informational",
        summary:
          "Strong candidate evidence supports the applicable requirement.",
