...
test("bloqueia POA&M sem tenant context", async () => {
  ...
  await expectRejects(
    () => new PoamDraftService(poam).createPoamDraft(context.assessmentId, approvedGap.gap_analysis_version_id, {}, { ...context, organizationId: "" }),
...
test("cria poam_version draft e itens rastreaveis para not_met", async () => {
  ...
  const items = await poam.repositories.items.listByVersion(draft.poam_version_id, context.organizationId);
...
test("gera milestones vinculados ao poam_item", async () => {
  ...
  const draft = await new PoamDraftService(poam).createPoamDraft(context.assessmentId, approvedGap.gap_analysis_version_id, {}, context);
  const items = await poam.repositories.items.listByVersion(draft.poam_version_id, context.organizationId);
  const milestones = await poam.repositories.milestones.listByItem(items[0]!.poam_item_id, context.organizationId);
