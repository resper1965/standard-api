...
test("documentation_gap gera policy_update ou procedure_creation", async () => {
  ...
  await updateGapFinding(gap, findings[0]!, {
    assessment_status: "partially_met",
    gap_type: "documentation_gap",
    severity: "low",
  });
  const draft = await new PoamDraftService(poam).createPoamDraft(
    context.assessmentId,
    approvedGap.gap_analysis_version_id,
    {},
    context,
  );
  const items = await poam.repositories.items.listByVersion(
    draft.poam_version_id,
    context.organizationId,
  );
  const item = items.find(
    (i) => i.related_gap_finding_id === findings[0]!.gap_finding_id,
  );
...
test("maturity score baixo aumenta prioridade", async () => {
  ...
  poam.maturity = {
    findApprovedOrDraftByAssessment: async () => ({
      maturity_assessment_version_id: "77777777-7777-4777-8777-777777777777",
      status: "approved",
    }),
    findScoreByControl: async () => ({
      maturity_score_id: "88888888-8888-4888-8888-888888888888",
      scf_control_id: findings[0]!.scf_control_id!,
      score: 1,
...
test("ausencia de maturity assessment permite POA&M com limitacao", async () => {
  ...
  expect(draft.metadata.limitations[0]).toBe(
    "Maturity Assessment not available; prioritization used Gap Analysis and SCF context only.",
  );
});
