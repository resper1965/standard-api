...
function controlCriticalityWeight(scfControl?: ScfControl | null): number {
  return scfControl?.control_description?.toLowerCase().includes("critical")
    ? 1
    : 0;
}
...
function calculateRiskScore(
  ...
  const controlWeight =
    scfControl?.control_weight != null
      ? Number(scfControl.control_weight)
      : 1.0;
  const severityFactor = severityWeight(gapFinding.severity);
  const maturityLevel = maturityScore?.score != null ? maturityScore.score : 0;
  const maturityFactor = Math.max(5 - maturityLevel, 0);
  return controlWeight * severityFactor * maturityFactor;
}
...
function resolveActionType(gapFinding: GapFindingResponse): PoamActionType {
  if (
    gapFinding.assessment_status === "requires_validation" ||
    gapFinding.assessment_status === "not_applicable_not_justified"
  )
    return "validation_required";
  if (gapFinding.assessment_status === "not_evidenced")
...
const SMALL_EFFORT_ACTIONS: PoamActionType[] = [
  "evidence_collection",
  "validation_required",
];
const MEDIUM_EFFORT_ACTIONS: PoamActionType[] = [
  "policy_update",
  "procedure_creation",
  "training",
];
...
export class PoamPrioritizationService {
  ...
  calculatePriorityScore(
    ...
    return calculateRiskScore(gapFinding, maturityScore, scfControl);
  }
  ...
  calculateRiskRating(
    ...
    const maturityText = maturityScore
      ? `maturity ${maturityScore.score}`
