...
import type { CreatePoamDraftOptions, GapFindingResponse, MaturityScoreReference, PoamDependencies, PoamItemFilters, PoamItemResponse, PoamVersionResponse, PoamContext } from "../types";
...
const actionableStatuses = new Set(["not_met", "partially_met", "not_evidenced", "requires_validation", "not_applicable_not_justified"]);
...
export class PoamDraftService {
  ...
  async createPoamDraft(assessmentId: string, gapAnalysisVersionId: string, options: CreatePoamDraftOptions = {}, context: PoamContext): Promise<PoamVersionResponse> {
    ...
    const version: PoamVersionResponse = {
      ...
      ...(maturityVersion ? { source_maturity_assessment_version_id: maturityVersion.maturity_assessment_version_id } : {}),
      ...
      metadata: {
        limitations,
        assumptions: ["Generated from approved Gap Analysis findings using conservative MVP rules."],
    ...
    await this.deps.repositories.dependencies.saveMany(this.scheduling.detectDependencies(items));
    return version;
  }

  async regeneratePoamDraft(poamVersionId: string, options: CreatePoamDraftOptions = {}, context: PoamContext): Promise<PoamVersionResponse> {
    const existing = await this.getPoamVersion(poamVersionId, context);
    if (existing.status === "approved") throw new PoamWorkflowError("POAM_IMMUTABLE", "Approved POA&M versions are immutable. Create a new draft version.");
    return this.createPoamDraft(existing.assessment_id, existing.source_gap_analysis_version_id, options, context);
  }
  ...
  private async generateItems(version: PoamVersionResponse, findings: GapFindingResponse[], maturityVersionId: string | undefined, context: PoamContext): Promise<PoamItemResponse[]> {
    const items: PoamItemResponse[] = [];
    for (const finding of findings) {
      ...
      const priority = this.prioritization.calculatePriority(finding, maturityScore, scfControl);
      ...
      items.push({
        ...
        ...(maturityScore ? { source_maturity_score_id: maturityScore.maturity_score_id } : {}),
        ...
        ...(scfControl?.scf_domain_id ? { scf_domain_id: scfControl.scf_domain_id } : {}),
        ...(finding.scf_control_id ? { scf_control_id: finding.scf_control_id } : {}),
        poam_code: `POAM-${String(items.length + 1).padStart(3, "0")}`,
        ...
        risk_rating: this.prioritization.calculateRiskRating(finding, maturityScore),
        effort_estimate: effort,
        owner_role: this.ownerRoleFor(actionType),
        due_date: this.scheduling.suggestDueDate(priority, effort),
        ...(maturityScore ? { target_maturity_score: this.targetMaturityScore(maturityScore) } : {}),
        expected_evidence: this.expectedEvidenceFor(actionType),
        acceptance_criteria: this.acceptanceCriteriaFor(finding, actionType),
        dependencies_summary: "No blocking dependency detected in MVP generation.",
        status: "draft",
        rationale: `Generated from ${finding.assessment_status} ${finding.gap_type} finding ${finding.gap_code}.`,
        confidence_score: finding.confidence_score,
        requires_user_validation: finding.requires_user_validation || finding.confidence_score < 0.5 || actionType === "validation_required",
        created_at: now,
        updated_at: now
      });
    }
  ...
  private correctiveActionFor(finding: GapFindingResponse, actionType: string): string {
    if (actionType === "evidence_collection") return `Collect and validate missing evidence for ${finding.gap_code} before concluding implementation status.`;
    if (actionType === "validation_required") return `Validate applicability and remediation need for ${finding.gap_code} with a human reviewer.`;
  ...
  private expectedEvidenceFor(actionType: string): string[] {
    if (actionType === "evidence_collection") return ["Accepted evidence artifact linked to the related SoA item.", "Reviewer note confirming evidence sufficiency."];
    if (actionType === "technical_implementation") return ["Implementation record or change ticket.", "Test or monitoring evidence showing the control operates as intended."];
    if (actionType === "monitoring_improvement") return ["Monitoring procedure or dashboard evidence.", "Review cadence evidence."];
    return ["Approved policy/procedure or validation record.", "Reviewer acceptance note."];
  }

  private acceptanceCriteriaFor(finding: GapFindingResponse, actionType: string): string[] {
    return [
  ...
  private ownerRoleFor(actionType: string): string {
    if (actionType === "technical_implementation" || actionType === "monitoring_improvement") return "control_owner";
