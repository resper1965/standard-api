/**
 * @module poam/repositories/drizzle.repository
...
import type {
  PoamVersionRepository,
  PoamItemRepository,
  PoamMilestoneRepository,
  PoamDependencyRepository,
  PoamRepositories,
} from "../types";
...
const mapPoamVersionRow = (row: PoamVersionRow): PoamVersionResponse => ({
  ...
  source_maturity_assessment_version_id:
    row.sourceMaturityAssessmentVersionId ?? undefined,
  ...
  metadata: {
    limitations:
      ((row.metadata as Record<string, unknown>)?.limitations as string[]) ??
      [],
    assumptions:
      ((row.metadata as Record<string, unknown>)?.assumptions as string[]) ??
      [],
    source_status:
      ((row.metadata as Record<string, unknown>)?.source_status as string) ??
      undefined,
  },
});
...
const mapPoamDependencyRow = (
  row: PoamDependencyRow,
): PoamDependencyResponse => ({
...
const createDrizzlePoamVersionRepository = (
  db: DrizzleDbClient,
): PoamVersionRepository => ({
  ...
  async get(poamVersionId, _organizationId) {
    const [row] = await db
      .select()
      .from(poamVersions)
...
const createDrizzlePoamItemRepository = (
  db: DrizzleDbClient,
): PoamItemRepository => ({
  ...
  async get(poamItemId, _organizationId) {
    const [row] = await db
      .select()
      .from(poamItems)
  ...
  async listByVersion(poamVersionId, _organizationId) {
    const rows = await db
      .select()
      .from(poamItems)
  ...
  withOrganization(organizationId) {
    return {
      ...
      listByVersion: async (poamVersionId) =>
        this.listByVersion(poamVersionId, organizationId),
    };
  },
});

const createDrizzlePoamMilestoneRepository = (
  db: DrizzleDbClient,
): PoamMilestoneRepository => ({
  ...
  async get(milestoneId, _organizationId) {
    const [row] = await db
      .select()
      .from(poamMilestones)
  ...
  async listByItem(poamItemId, _organizationId) {
    const rows = await db
      .select()
      .from(poamMilestones)
  ...
  withOrganization(organizationId) {
    return {
      ...
      listByItem: async (poamItemId) =>
        this.listByItem(poamItemId, organizationId),
    };
  },
});

const createDrizzlePoamDependencyRepository = (
  db: DrizzleDbClient,
): PoamDependencyRepository => ({
  async save(dep: PoamDependencyResponse) {
    await db
      .insert(poamDependencies)
      .values({
...
/**
 * Factory: creates all Drizzle-backed POAM repositories.
 * Pass the DbClient from the api-gateway composition root.
 */
export const createDrizzlePoamRepositories = (
  db: DrizzleDbClient,
): PoamRepositories => ({
