...
test("createMaturityDraft incorporates official rubrics when available", async () => {
  const versionsStore: any[] = [];
  const scoresStore: any[] = [];
  ...
  const deps: MaturityDependencies = {
    ...
    async getMaturityCriteriaForControl(controlId) {
      return [
        {
          level: 5,
          criteriaText: "Continuous optimization through automation",
          remediationGuidance: "Ensure weekly reviews",
        },
      ];
    },
  };
  ...
  expect(scoresStore.length).toBe(1);
  const score1 = scoresStore[0];
  expect(score1.score).toBe(5);
  expect(
    score1.rationale.includes("Continuous optimization through automation"),
  ).toBe(true);
  expect(score1.rationale.includes("Ensure weekly reviews")).toBe(true);
});
