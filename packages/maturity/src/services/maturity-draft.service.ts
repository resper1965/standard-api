...
export const createMaturityDraft = async (
  ...
  const gapResult = await deps.getApprovedGapAnalysis(
    ctx.assessmentId,
    ctx.organizationId,
  );
  if (!gapResult) {
  ...
  const existingVersions = await deps.repositories.versions.listByAssessment(
    ctx.assessmentId,
    ctx.organizationId,
  );
  ...
  const scores: MaturityScore[] = await Promise.all(
    scorableFindings.map(async (finding) => {
      ...
      const rubrics = deps.getMaturityCriteriaForControl
        ? await deps.getMaturityCriteriaForControl(finding.scf_control_id!)
        : undefined;
...
export const computeSummary = (scores: MaturityScore[]): MaturitySummary => {
  ...
  const levelDistribution: Record<MaturityLevel, number> = {
    0: 0,
    1: 0,
    2: 0,
    3: 0,
