 ...
 */
export const classifyMaturity = (input: MaturityClassificationInput): ClassificationResult => {
  ...
  if (input.gapStatus === "not_applicable_justified" || input.gapStatus === "not_applicable_not_justified") {
    return {
      score: 0,
  ...
  const hasEvidence = input.evidenceStrength === "strong" || input.evidenceStrength === "partial";
  ...
  // Level 3: documented process with strong evidence
  if (input.hasDocumentation && input.hasProcess && input.evidenceStrength === "strong") {
