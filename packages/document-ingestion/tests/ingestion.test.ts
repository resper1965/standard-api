...
test("validação rejeita tipo não permitido", async () => {
  await expectRejects(
    () => validateFile({ originalFilename: "malware.exe", mimeType: "application/x-msdownload", bytes: textBytes("x") }).then(() => undefined),
...
test("filename é sanitizado", () => {
  expect(sanitizeFilename("../Relatório Cliente 01!!.txt")).toBe("relatorio-cliente-01.txt");
});
...
test("PlainTextExtractor extrai texto", async () => {
  const extracted = await new PlainTextExtractor().extract({ bytes: textBytes("hello"), mimeType: "text/plain", extension: "txt", filename: "hello.txt" });
...
test("erro no extractor marca job como failed com safe error message", async () => {
  const failingExtractor: DocumentTextExtractor = {
    supports: () => true,
    async extract() {
      throw new Error("conteúdo sensível não deve aparecer completo ".repeat(20));
    }
  };
  const deps = createInMemoryDocumentIngestionDependencies({ extractors: [failingExtractor] });
  const service = new DocumentIngestionService(deps);
  const result = await service.uploadDocument({
    documentId: ids.documentId,
    jobId: ids.jobId,
