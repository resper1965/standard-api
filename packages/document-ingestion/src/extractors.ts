...
export class AzurePdfExtractor implements DocumentTextExtractor {
  ...
  async extract(input: { bytes: Uint8Array; filename: string }): Promise<ExtractedDocument> {
    if (!this.apiKey || !this.endpoint) {
      console.warn("Azure endpoints not configured. Using dummy fallback extraction.");
      return {
    ...
    try {
      const response = await fetch(`${this.endpoint}/documentModels/prebuilt-read:analyze?api-version=2023-07-31`, {
        method: "POST",
        headers: {
      ...
      if (!operationLocation) throw new Error("Missing Operation-Location header from Azure.");

      // Poll for completion
      let parsedText = "";
      for (let i = 0; i < 15; i++) {
        await new Promise(r => setTimeout(r, 2000));
        const poll = await fetch(operationLocation, { headers: { "Ocp-Apim-Subscription-Key": this.apiKey }});
        const result = await poll.json() as any;
        if (result.status === "succeeded") {
          parsedText = result.analyzeResult?.content || "No content found.";
          break;
        }
        if (result.status === "failed") throw new Error("Azure Extraction Failed");
      }

      return { text: parsedText, metadata: { extractor: "azure_doc_intelligence" }, warnings: [] };
    } catch (e) {
      console.error(e);
...
export class WebhookOcrExtractor implements DocumentTextExtractor {
  ...
  supports(mimeType: string, extension: string): boolean {
    ...
    return ["pdf", "png", "jpg", "jpeg", "tif", "tiff", "doc", "docx"].includes(extension.toLowerCase()) || 
           mimeType.startsWith("image/") || 
           mimeType === "application/pdf" ||
           mimeType.includes("word");
  }

  async extract(input: { bytes: Uint8Array; mimeType: string; filename: string }): Promise<ExtractedDocument> {
    if (!this.endpoint) {
    ...
    try {
      ...
      formData.append("files", new Blob([input.bytes as any], { type: input.mimeType }), input.filename);
      ...
      if (!response.ok) {
        throw new Error(`Webhook OCR failed: ${response.status} ${response.statusText}`);
      }
      ...
      if (contentType.includes("application/json")) {
        ...
        parsedText = Array.isArray(json) ? json.map(e => e.text).join("\n\n") : (json.text || JSON.stringify(json));
      } else {
        parsedText = await response.text();
      }

      return { text: parsedText, metadata: { extractor: "webhook_ocr" }, warnings: [] };
    } catch (e) {
      console.error(e);
      throw new Error(`Failed to extract via Webhook OCR: ${(e as Error).message}`);
    }
  }
}
...
export const getDefaultExtractors = (env?: Record<string, string>) => {
  ...
  } else if (env?.AZURE_DOCUMENT_INTELLIGENCE_KEY && env?.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT) {
    defaults.push(new AzurePdfExtractor(env.AZURE_DOCUMENT_INTELLIGENCE_KEY, env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT));
  } else {
