...
export class SoaDraftService {
  ...
  async createDraftFromFramework(
    assessmentId: string,
    frameworkId: string,
    scfVersionId: string,
    ...
    const requirements =
      await this.deps.scf.frameworks.listRequirements(frameworkId);
    const mappings = await this.deps.scf.mappings.mapFrameworkToScf(
      frameworkId,
      scfVersionId,
    );
    return this.createDraftFromRequirements(
      { assessmentId, frameworkId, scfVersionId, requirements, mappings },
      context,
    );
  }

  async createDraftFromRequirements(
    ...
    const existing = await this.deps.repositories.versions.listByAssessment(
  ...
  regenerateDraft(
    assessmentId: string,
    options: { frameworkId: string; scfVersionId: string },
    context: SoaWorkflowContext,
  ): Promise<SoaVersionResponse> {
    return this.createDraftFromFramework(
      assessmentId,
      options.frameworkId,
      options.scfVersionId,
      context,
    );
  }
  ...
  async listSoaItems(
    ...
    return filters.applicability_status
      ? items.filter(
          (item) => item.applicability_status === filters.applicability_status,
        )
      : items;
  }
  ...
  private createItem(
    ...
    return {
      ...
      validation_notes:
        mapping.mappingStatus === "no_official_mapping"
          ? "No official SCF mapping is available for this requirement."
          : "Requires applicability validation.",
      ...(mapping.sourceMappingId
        ? { source_mapping_id: mapping.sourceMappingId }
        : {}),
      mapping_status: mapping.mappingStatus,
      ...(mapping.relationshipType
        ? { relationship_type: mapping.relationshipType }
        : {}),
      ...(mapping.relationshipStrength
        ? { relationship_strength: mapping.relationshipStrength }
        : {}),
