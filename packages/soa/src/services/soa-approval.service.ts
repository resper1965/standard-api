...
export class SoaApprovalService {
  ...
  async validateSoaForApproval(
    soaVersionId: string,
    context: SoaWorkflowContext,
  ): Promise<SoaValidationResponse> {
    return new SoaReviewService(this.deps).validateSoaForReview(
      soaVersionId,
      context,
    );
  }
  ...
  async approveSoa(
    ...
    if (!approval.approval_event_id)
      throw new SoaWorkflowError(
        ...
        "SoA approval requires a human approval event.",
      );
    const version = await this.getVersion(soaVersionId, context);
    if (version.status !== "under_review")
      throw new SoaWorkflowError(
        "SOA_APPROVAL_BLOCKED",
        "Only under_review SoA versions can be approved.",
      );
    ...
    await this.supersedePreviousApprovedVersions(
      context.assessmentId,
      (approved as any).soa_version_id as string,
      context,
    );
    return approved;
  }

  async supersedePreviousApprovedVersions(
    assessmentId: string,
    approvedSoaVersionId: string,
    context: SoaWorkflowContext,
  ): Promise<void> {
    ...
    for (const version of versions) {
      if (
        version.soa_version_id !== approvedSoaVersionId &&
  ...
  async markSoaIngestionRequired(
    soaVersionId: string,
    context: SoaWorkflowContext,
  ): Promise<SoaVersionResponse> {
    const version = await this.getVersion(soaVersionId, context);
