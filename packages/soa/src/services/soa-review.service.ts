...
export class SoaReviewService {
  ...
  async updateSoaItemDecision(
    soaItemId: string,
    patch: UpdateSoaItemRequest,
    ...
    if (!item || item.assessment_id !== context.assessmentId)
      throw new SoaWorkflowError("SOA_ITEM_NOT_FOUND", "SoA item not found.");
    const version = await this.getVersion(
      (item as any).soa_version_id,
      context,
    );
    if (version.status === "approved")
      throw new SoaWorkflowError(
        "SOA_VERSION_IMMUTABLE",
        "Approved SoA versions are immutable.",
      );
    const cleanPatch = definedPatch(patch) as Partial<SoaItemResponse>;
    const candidate: SoaItemResponse = {
      ...item,
      ...cleanPatch,
  ...
  async bulkUpdateApplicability(
    soaVersionId: string,
    items: Array<{ soa_item_id: string; patch: UpdateSoaItemRequest }>,
    context: SoaWorkflowContext,
  ): Promise<SoaItemResponse[]> {
    const updated: SoaItemResponse[] = [];
    for (const item of items)
      updated.push(
        await this.updateSoaItemDecision(item.soa_item_id, item.patch, context),
      );
    return updated.filter((item) => item.soa_version_id === soaVersionId);
  }

  async validateSoaForReview(
    soaVersionId: string,
    context: SoaWorkflowContext,
  ): Promise<SoaValidationResponse> {
    assertContext(context);
    const items = await this.deps.repositories.items.listByVersion(
      soaVersionId,
      context.organizationId,
    );
    ...
    return {
      ...
      warnings: items.some((item) => item.evidence_coverage === "not_checked")
        ? ["Some items have not checked evidence coverage."]
        : [],
      trace_id: context.traceId,
    };
  }

  async submitSoaForReview(
    soaVersionId: string,
    context: SoaWorkflowContext,
    ...
    const hasToBeDefined = (validation as any).blocking_errors.some(
      (error: string) => error.includes("to_be_defined"),
    );
    if (!validation.valid && !(hasToBeDefined && exceptionRationale)) {
      throw new SoaWorkflowError(
        "SOA_REVIEW_BLOCKED",
        "SoA has blocking validation errors.",
  ...
  private itemReviewErrors(item: SoaItemResponse): string[] {
    const errors: string[] = [];
    if (item.applicability_status === "to_be_defined")
      errors.push(`Item ${item.soa_item_id} is to_be_defined.`);
    if (
      item.applicability_status === "not_applicable" &&
      !item.non_applicability_rationale
    )
      errors.push(
        `Item ${item.soa_item_id} missing non_applicability_rationale.`,
      );
    if (item.applicability_status === "out_of_scope" && !item.scope_rationale)
      errors.push(`Item ${item.soa_item_id} missing scope_rationale.`);
    return errors;
  }
  ...
  private async getVersion(
    ...
    if (!version || version.assessment_id !== context.assessmentId)
      throw new SoaWorkflowError(
        "SOA_VERSION_NOT_FOUND",
        "SoA version not found.",
      );
    return version;
  }
}
