...
| --- | --- | --- | --- | --- | --- |
| `AEGIS_DOCUMENTS_BUCKET` | R2 | dev/staging/prod | API, ingestion, KB worker | Sim | Documentos e evidências com keys escopadas por tenant/assessment. |
| `AEGIS_REPORTS_BUCKET` | R2 | dev/staging/prod | API, reporting worker | Sim | Relatórios e artefatos versionados. |
| `AEGIS_EXPORTS_BUCKET` | R2 | dev/staging/prod | API, reporting worker | Opcional | Pode ser unido ao bucket de reports no MVP. |
...
| `AEGIS_KB_INDEX` | Vectorize | dev/staging/prod | ingestion, KB worker | Sim para KB real | Índice vetorial auxiliar; SCF continua fonte normativa. |
| `AEGIS_CONFIG_KV` | KV | dev/staging/prod | API | Opcional | Configuração leve não crítica. |
| `AEGIS_FEATURE_FLAGS_KV` | KV | dev/staging/prod | API | Opcional | Feature flags por ambiente. |
| `AEGIS_CACHE_KV` | KV | dev/staging/prod | API | Opcional | Cache edge de baixo risco. |
