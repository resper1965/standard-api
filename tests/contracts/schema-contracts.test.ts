...
test("SoA item sem mapping oficial pode exigir validação", () => {
  const item = SoaItemResponseSchema.parse({
    ...
    applicability_rationale:
      "No official mapping exists in the synthetic SCF fixture.",
...
test("audit e security event contracts exigem trace_id", () => {
  expect(TraceIdSchema.parse("trace-contract-0001")).toBe(
    "trace-contract-0001",
  );
  expect(
  ...
  ).toBe("trace-contract-0001");
  expect(
    SecurityEventRecordSchema.parse({
